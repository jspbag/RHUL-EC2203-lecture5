...
- Assume for now that we have a (consistent) estimator $\hat{\sigma}^2_{\hat{\beta}_1}$ of the unknown parameter $\sigma^2_{\hat{\beta}_1}$
...
- Suppose we hypothesize that the unknown population parameter $\beta_1$ is equal to some specific value $\beta_{1,0}$ 

  $$H_0: \, \beta_1 = \beta_{1,0}; \quad H_1: \, \beta_1 \neq \beta_{1,0}$$

  For example, hypothesize $\beta_1 = 0$: there is no causal effect of class size on test scores

- We can estimate $\beta_1$ by the OLS estimator $\hat{\beta}_1$; but, how can we reject (or not) $H_0: \, \beta_1 = \beta_{1,0}$? 
...
- When $H_0$ is true, $\hat{\beta}_1 \overset{\text{approx}}{\sim} \mathcal{N}(\beta_{1,0},\hat{\sigma}^2_{\hat{\beta}_1})$. 
...
- Fix **significance level** $\alpha$ (e.g. $\alpha = 0.05$) and apply the rule:
  ...
  <div class="box">
  Reject $H_0$ if $|t| > z_{1-\alpha/2}$, where **critical value** $z_{1-\alpha/2}$ is $(1-\alpha/2)\times 100$ p'tile in $\mathcal{N}(0,1)$
  </div>
...
axis(1, at = c(0, -1.96, 1.96, -tact, tact,-4.39,4.39))
...
- The OLS estimator $\hat{\beta}_1$ is a random variable 

- Use a $t$-test to test null hypotheses about the parameter $\beta_1$ in the population regression model

- The critical value for the $t$-statistic for testing on a 5% significance level is 1.96: reject a null hypothesis when $|t| > 1.96$, or equivalently when $p$-value $< 0.05$  

- To compute the $t$-statistic we need an estimate of the variance of the OLS estimator $\hat{\beta}_1$
